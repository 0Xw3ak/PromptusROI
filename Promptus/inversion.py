...
def load_basnet_model(ckpt_path: str):
    if not os.path.exists(ckpt_path):
        raise FileNotFoundError("BASNet checkpoint not found: {}".format(ckpt_path))
    basnet_root = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "BASNet"))
    if basnet_root not in sys.path:
        sys.path.insert(0, basnet_root)
    from model import BASNet as BASNetModel

    basnet = BASNetModel(3, 1)
    state_dict = torch.load(ckpt_path, map_location="cpu")
    basnet.load_state_dict(state_dict)
    basnet = basnet.cuda().eval()
    for param in basnet.parameters():
        param.requires_grad = False
    return basnet


def preprocess_for_basnet(img: torch.Tensor) -> torch.Tensor:
    # BASNet expects ImageNet-normalized RGB inputs in [N,3,256,256].
...
def sanitize_prob_map(x: torch.Tensor) -> torch.Tensor:
    # BCE on CUDA requires finite probabilities in [0, 1].
...
def inversion(
        ...
        roi_enable=True,
        roi_weight=0.2,
        roi_basnet_ckpt="../BASNet/saved_models/basnet_bsi/basnet.pth"
):
    F = 8
    C = 4
    shape = (1, C, H // F, W // F)
    ...
    with precision_scope("cuda"):
        ...
        roi_loss_fn = nn.BCELoss()
        basnet = None
        if roi_enable:
            basnet = load_basnet_model(roi_basnet_ckpt)
            print("ROI loss enabled. Loaded BASNet from: {}".format(roi_basnet_ckpt))
        gt_roi_cache = {}
        ...
        for f_id in range(0, max_id, interval):
            ...
            for iter in range(total_iterations):
                ...
                for step in step_list:
                    ...
                    mse_value = mse_loss(samples_x, gt)
                    # Combine the perceptual loss and reconstruction loss.
                    loss = 0.2 * lpips_loss + 0.8 * mse_value

                    # regularization
                    loss_regu = torch.mean(torch.abs(c['crossattn']))
                    loss = loss + 0.1 * loss_regu
                    roi_loss = torch.tensor(0.0, device=samples_x.device, dtype=samples_x.dtype)
                    if roi_enable:
                        with torch.amp.autocast("cuda", enabled=False):
                            roi_pred, _, _, _, _, _, _, _ = basnet(preprocess_for_basnet(samples_x.float()))
                            roi_pred = sanitize_prob_map(roi_pred)
                            if cur_id not in gt_roi_cache:
                                with torch.no_grad():
                                    roi_gt_cached, _, _, _, _, _, _, _ = basnet(preprocess_for_basnet(gt.float()))
                                    gt_roi_cache[cur_id] = sanitize_prob_map(roi_gt_cached.detach())
                            roi_loss = roi_loss_fn(roi_pred, gt_roi_cache[cur_id])
                        loss = loss + roi_weight * roi_loss

                    # logging
                    print('iter: {}, cur_id: {}, loss: {}, lpips: {}, mse: {}, regu: {}, roi: {}, c_max: {}, c_mean: {}, c_std: {}'.format(
                        iter, cur_id, loss, lpips_loss, mse_value, loss_regu, roi_loss,
                        c['crossattn'].max(), c['crossattn'].mean(), c['crossattn'].std()
                    ))
                    log_output.write('iter: {}, cur_id: {}, loss: {}, lpips: {}, mse: {}, regu: {}, roi: {}, c_max: {}, c_mean: {}, c_std: {}\n'.format(
                        iter, cur_id, loss, lpips_loss, mse_value, loss_regu, roi_loss,
                        c['crossattn'].max(), c['crossattn'].mean(), c['crossattn'].std()
                    ))
                    log_output.flush()
...
if __name__ == "__main__":
    ...
    parser.add_argument('--roi_enable', type=int, default=1)
    parser.add_argument('--roi_weight', type=float, default=0.2)
    parser.add_argument('--roi_basnet_ckpt', type=str, default="../BASNet/saved_models/basnet_bsi/basnet.pth")
    ...
    inversion(
       ...
       roi_enable=bool(args.roi_enable), roi_weight=args.roi_weight, roi_basnet_ckpt=args.roi_basnet_ckpt
    )
